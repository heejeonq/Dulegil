...
public class AOPComponent {
	...
	public ModelAndView adNtAOP(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		if (session.getAttribute("adMemNm") != null && session.getAttribute("adMemNm") != "") { // 로그인중일 경우
	...
	@Pointcut("execution(* com.gdj51.Dulegil..*Controller.*(..))"
			...
			+ "&& !execution(* com.gdj51.Dulegil..*Controller.*List(..))"
			+ "&& !execution(* com.gdj51.Dulegil..*Controller.*Detail(..))"
			...
			+ "&& !execution(* com.gdj51.Dulegil..JoinController.*(..))")
	public void DulAOP() {
	};
	...
	public ModelAndView DulAOP(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		if (session.getAttribute("sMemNm") != null && session.getAttribute("sMemNm") != "") { // 로그인중일 경우
			mav = (ModelAndView) joinPoint.proceed(); // 기존 이벤트 처리 행위를 이어서 진행
		} else { // 비로그인인 경우
